...
//added
static wait_queue_head_t * waitQueues;
	    ...
	    loff_t *f_pos)
{
  ...
  dn = (filp->f_path.dentry->d_iname)[6] - '0';
  
  flags[dn] = 1;
  wake_up_interruptible(&waitQueues[dn]);
...
sleepy_write(struct file *filp, const char __user *buf, size_t count, 
	     loff_t *f_pos)
{
  struct sleepy_dev *dev;
  ssize_t retval = 0;
	int secs = 0;
	int dn;
	unsigned long jiffies;
  unsigned long tr;
  wait_queue_head_t * curr;
	dev  = (struct sleepy_dev *)filp->private_data;
  if (mutex_lock_killable(&dev->sleepy_mutex))
	{
    return -EINTR;
	}
	...
	copy_from_user(&secs, buf, count);

	//get the number of device we are on
  dn = (filp->f_path.dentry->d_iname)[6] - '0';
  
  jiffies = msecs_to_jiffies((secs * 1000));	

  curr = &waitQueues[dn];  
  
  mutex_unlock(&dev->sleepy_mutex);
  
  tr = wait_event_interruptible_timeout(*curr, flags[dn] != 0, jiffies);
  flags[dn] = 0;
  
  retval = jiffies_to_msecs(tr) / 1000;
...
sleepy_init_module(void)
{
  ...
  waitQueues = kmalloc(sizeof(wait_queue_head_t)*sleepy_ndevices, GFP_KERNEL);
